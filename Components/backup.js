...
const ChatComponent = ({ navigation, chatType, data, socket }) => {
    ...
    useEffect(() => {
        ...
        socket.emit('get-messages', {type: chatType === 'cgrp' ? 'group' : 'buddy', token: userToken, displayId: data.displayId, chatId: data.id}, (response) => {
            response = JSON.parse(response);
            if(chatType === 'cgrp') {
                setGrpList(response.data.user.map(function (val) {
                    return val.name;
                }));
            }
            if(chatType != 'cgrp') {
                setDistance(response.data.user[0].distance);
            }
            setMore(response.more);
            matchLocals(response, Contacts, 'getMessages')
        })
        socket.on('isTypingUser', (response) => {
            response = JSON.parse(response);
            if((chatType == 'cgrp' && response.type == 'group' && response.displayId == data.displayId) || response.fromId == data.displayId) {
                setisTyping(true);
                setTypingMsg(response.msg);
        ...
        socket.on('newMesssageReceived', (response) => {
            response = JSON.parse(response);
            setNewMessage(response);
        });
        matchLocals(NewMessage, 'newMesssageReceived')
    }, [Contacts]);
    ...
    const matchLocals = async(data, type) => {
        if(type === 'getMessages') {
            if(value != null) {
                for (var i = 0; i < value.length; i++) {
                    if(value[i].idNormal == data.data.chat.displayId) {
                        data.data.chat.displayName = value[i].name;
                    }
                }
                for (var i = 0; i < value.length; i++) {
                    for (var j = 0; j < data.data.messages.length; j++) {
                        if (value[i].idNormal == data.data.messages[j].from_id) {
                            data.data.messages[j].displayName = value[i].name;
                        } 
                    }
                }
        ...
        if(type === 'newMesssageReceived') {
            if(data != null) {
                for (var i = 0; i < value.length; i++) {
                    if(value[i].idNormal == data.data.message[0].from_id) {
                        data.data.message[0].displayName = value[i].name;
                    }
                }
    ...
    const onKeyPress = () => {
        socket.emit('send-typing', {type: chatType === 'cgrp' ? 'group' : 'buddy', token: userToken, displayId: data.displayId, chatId: data.id})
    }
    ...
    const sendMessage = (msg, type) => {
        if(msg != null) {
            ...
            socket.emit('send-message', 
                {
                    type: chatType === 'cgrp' ? 'group' : 'buddy', 
                    token: userToken, 
                    displayId: data.displayId, 
                    chatId: data.id, 
                    text: type === 'image' ? null : msg, 
                    image: type === 'image' ? {blob: msg.data, name: msg.filename} : null
                }, (response) => {
                response = JSON.parse(response);
                let newData = [...chatData];
                newData.unshift(response.data.message[0]);
    ...
    const getMoreMessages = () => {
        if(More != '') {
            setRefresh(true);
            socket.emit('get-messages', {type: chatType === 'cgrp' ? 'group' : 'buddy', token: userToken, displayId: data.displayId, chatId: data.id, offset: parseInt(More)}, (response) => {
                response = JSON.parse(response);
                matchLocals(response, 'getMoreMessages')
            })
        }
    }
            ...
            />
            {
                ...
                <View style={{position: 'relative', flex: 1, backgroundColor: theme.dark ? '#212329' : Colors.gray}} >
                    ...
                    >
                        ...
                        >
                            <Image 
                                source={{uri: `${Api.baseurl}/${userData.displayImage}`}} 
                                resizeMode='cover' 
                                        ...
                                        <View style={[styles.ChatListBox, item.isOwner === 'yes' ? styles.ChatListBoxSelf : null,
                                            ...
                                            <View style={[styles.ChatListBoxInner, item.isOwner === 'yes' ? styles.ChatListBoxInnerSelf : null, {backgroundColor: colors.background}]}>
                                                ...
                                                }
                                                {
                                                    ...
                                                    >
                                                        <ImageScale 
                                                            source={{uri: `${Api.baseurl}/n-api${item.text}`}} 
                                                        />
                                                    </TouchableOpacity> :
