...
const NotificationScreen = ({navigation, route}) => {
  ...
  const getFilteredData = () => {
    ...
    if (activeFilter === 'Requests')
      return notificationData.filter((i) => i.type === 'request');
    if (activeFilter === 'Activity')
      return notificationData.filter((i) => i.type === 'interaction');
        ...
        <View
          ...
          <View style={styles.AvatarWrap}>
            ...
            <View
              style={[
                styles.AvatarBadge,
                {backgroundColor: isRequest ? '#7FFFD4' : '#2A2B2E'},
              ]}>
              <Feather
                name={isRequest ? 'user-plus' : 'bell'}
                size={11}
                color={isRequest ? '#09090B' : 'rgba(255,255,255,0.7)'}
              />
            </View>
          </View>
            ...
            <View style={styles.CardTopRow}>
              <View
                ...
                <Text
                  style={[
                    styles.RoleText,
                    {color: isRequest ? '#7FFFD4' : 'rgba(255,255,255,0.45)'},
                  ]}>
                  {item.role}
                </Text>
              </View>
  ...
  const renderRadiusSheet = () => (
    <View style={styles.BsContentCard}>
      ...
      <Text style={styles.BsSubtitle}>
        Select the distance (km) for proximity alerts
      </Text>
      ...
      <View style={styles.RadiusGrid}>
        {distance.map((item, index) => (
          <TouchableOpacity
            key={index}
            activeOpacity={0.8}
            style={[styles.RadiusBox, item.selected && styles.RadiusBoxActive]}
  ...
  return (
    <View style={styles.Screen}>
      ...
      <View
        style={[
          styles.Header,
          {paddingTop: insets.top + (deviceType === 'Tablet' ? 8 : 12)},
        ]}>
        <TouchableOpacity
      ...
      <View
        style={{
          flex: 1,
          marginTop: insets.top + (deviceType === 'Tablet' ? 65 : 78),
        }}>
        {/* Filter tabs */}
...
const styles = EStyleSheet.create({
  ...
  AvatarWrap: {position: 'relative', marginRight: '14rem', alignSelf: 'flex-start'},
  AvatarImg: {
    width: '52rem',
    height: '52rem',
    borderRadius: '26rem',
    ...
    AvatarBadge: {
        position: 'absolute', bottom: '-1rem', right: '-3rem', width: '22rem', height: '22rem',
  ...
  BsContentCard: {
    ...
    borderTopLeftRadius: '32rem',
    borderTopRightRadius: '32rem',
    padding: '24rem',
    borderWidth: 1,
    borderColor: 'rgba(255,255,255,0.08)',
    height: '420rem',
